...
function setup_firewall() {
  if ! iptables -nL | grep 1194 >/dev/null 2>&1; then
    ...
    color_echo title "$(display_hint "h_setup_firewall_setup"): Ping,SSH,1194udp"
  else
    color_echo title "$(display_hint "h_setup_firewall_enabled"): Ping,SSH,1194udp"
  fi
}
...
color_echo info "$(display_hint "h_complete_install_firewall"): Ping,SSH,1194udp"
